...
import { getToolSpecifications } from './toolConfig.js';
...
- \`getWeather\` tool is to get weather for a city dynamically. It needs a city to be provided as a location.
- \`agentSearch\` tool is to perform a Web Search. Use this tool when the user asks for an external or an agent search to be conducted. Do NOT use your internal memory to respond when the user is ASKING for an external or Google search. \`agentSearch\` takes time, so it will start the search and return. When user asks for external or agent search results invoke this tool again. When you hear the words "check search results" - please invoke this tool.
