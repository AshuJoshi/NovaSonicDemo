import { WebSocketEventManager } from "./websocketEvents_extension.js";
...
async function captureAndSendScreenshotForAnalysis(analysisId) {
  console.log(`MAIN: Attempting to capture screenshot for analysisId: ${analysisId}`);
  try {
    // Attempt to find the most relevant active tab that is not an extension page
    const tabs = await chrome.tabs.query({active: true, currentWindow: true});
    let targetTab = null;
    if (tabs && tabs.length > 0) {
        // Prioritize non-extension URLs if multiple active tabs (shouldn't happen with currentWindow:true)
        targetTab = tabs.find(t => t.url && !t.url.startsWith("chrome-extension://") && !t.url.startsWith("chrome://"));
        if (!targetTab) targetTab = tabs[0]; // Fallback to the first active tab
    }

    if (!targetTab || targetTab.id == null) {
      console.error("MAIN: No suitable active tab found to capture.");
      if (wsManager) wsManager.sendScreenshotDataToBackend(analysisId, null, "No active tab found");
      return;
    }

    console.log(`MAIN: Capturing visible tab ID: ${targetTab.id} (URL: ${targetTab.url})`);
    // The captureVisibleTab API captures the visible contents of the window that the specified tab is in.
    // If you want just the tab's content, and the tab might not be the full window, this can be tricky.
    // For a side panel, it usually means capturing the main window area.
    const imageDataUrl = await chrome.tabs.captureVisibleTab(targetTab.windowId, { format: "jpeg", quality: 80 });

    if (imageDataUrl && wsManager) {
      wsManager.sendScreenshotDataToBackend(analysisId, imageDataUrl);
    } else if (!imageDataUrl) {
      console.error("MAIN: Failed to capture screenshot (imageDataUrl is null/undefined).");
      if (wsManager) wsManager.sendScreenshotDataToBackend(analysisId, null, "Capture failed, no data URL");
    }
  } catch (e) {
    console.error("MAIN: Error during screenshot capture or send:", e);
    if (wsManager) {
      wsManager.sendScreenshotDataToBackend(analysisId, null, `Capture error: ${e.toString()}`);
    }
  }
}
// Expose globally for WebSocketEventManager to call
window.captureAndSendScreenshotForAnalysis = captureAndSendScreenshotForAnalysis;
